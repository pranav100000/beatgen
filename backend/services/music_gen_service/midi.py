...
class MIDIGenerator:
    ...
    async def generate_midi_separate(
        self, music_description: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def generate_midi(self, music_description: Dict[str, Any]) -> Dict[str, Any]:
        ...
        file_path = os.path.join(
            self.output_dir, f"{self._sanitize_filename(title)}.mid"
        )
        midi_file.save(file_path)
    ...
    def _add_instrument_track(
        self, midi_file: MidiFile, instrument: Dict[str, Any], tempo: int
    ) -> None:
        """
        ...
        # Add track name
        track.append(
            mido.MetaMessage("track_name", name=instrument.get("name", "Track"), time=0)
        )
    ...
    def _add_pattern_notes(self, track: MidiTrack, pattern: Dict[str, Any]) -> None:
        ...
        for note in notes:
            ...
            events.append(
                {"type": "note_on", "time": start, "pitch": pitch, "velocity": velocity}
            )
