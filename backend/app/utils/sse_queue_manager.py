...
class SSEQueueManager:
    ...
    def __init__(self, request_id: str):
        ...
        logger.debug(f"Created SSEEventQueue for request {request_id} with message {self.message_id}")
    ...
    async def add_chunk(self, text: str):
        ...
        if not self.is_started:
            logger.debug(f"Auto-starting stream for add_chunk")
            await self.start_stream()
            
        if self.is_ended:
            logger.warning(f"Attempting to add chunk after stream ended: {text[:30]}...")
            return
            
        await self.queue.put(("response_chunk", {
            "message_id": self.message_id,
            "chunk": text,
    ...
    async def end_stream(self):
        ...
        if not self.is_started:
            logger.warning(f"Ending stream that was never started for message {self.message_id}")
            await self.start_stream()
            
        await self.queue.put(("response_end", {
            "message_id": self.message_id,
            "is_complete": True
        }))
        
        self.is_ended = True
        logger.debug(f"Ended stream for message {self.message_id} after {self.chunk_index} chunks")
