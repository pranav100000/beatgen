            ...
            while True:
                ...
                try:
                    ...
                    if queue_size > 0:
                        ...
                        logger.debug(f"Queue has items, getting next event immediately")
                        ...
                        logger.debug(f"Queue empty, waiting for next event (timeout: {self.heartbeat_interval}s)")
                        try:
                    ...
                    try:
                        error_msg = self.format_event(
                            "error",
                            {"message": f"Error in event processing: {str(e)}"}
                        )
                        yield error_msg
                    except:
