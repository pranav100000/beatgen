    ...
    def _initialize_client(cls):
        ...
        try:
            if not settings.supabase.URL or not settings.supabase.KEY:
                ...
                logger.critical(f"SUPABASE_URL: {'SET' if settings.supabase.URL else 'MISSING'}")
                logger.critical(f"SUPABASE_KEY: {'SET' if settings.supabase.KEY else 'MISSING'}")
                ...
                cls._client.table.side_effect = Exception("Supabase client failed to initialize")
                return
                
            logger.info(f"Initializing Supabase client with URL: {settings.supabase.URL[:20]}...")
