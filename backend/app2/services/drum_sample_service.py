...
from app2.models.public_models.drum_samples import DrumSamplePublic, DrumSamplePublicRead
...
class DrumSampleService:
    ...
    async def create_sample_record(self, sample_data: Dict[str, Any]) -> DrumSamplePublicRead:
        """
    ...
    async def update_sample_record(self, sample_id: uuid.UUID, sample_data: Dict[str, Any]) -> DrumSamplePublicRead:
        """
        ...
        """
        logger.info(f"Updating drum sample record ID {sample_id} with data: {sample_data}")
        try:
            # Potentially validate sample_data against a Pydantic Update model here
            updated_sample = await self.drum_sample_repository.update(sample_id, sample_data)
            return DrumSamplePublicRead.model_validate(updated_sample)
        except NotFoundException:
            logger.warning(f"Drum sample record with ID {sample_id} not found for update.")
            raise
        except Exception as e:
    ...
    async def delete_sample_record(self, sample_id: uuid.UUID) -> bool:
        ...
        try:
            ...
            logger.info(f"Deletion result for drum sample record {sample_id}: {deleted}")
            return deleted
        except NotFoundException:
            logger.warning(f"Drum sample record with ID {sample_id} not found for deletion.")
            raise
        except Exception as e:
    ...
    async def create_sample_upload_url(self, file_name: str, file_id: uuid.UUID) -> Dict[str, str]:
        """
        ...
        """
        logger.info(f"Creating upload URL for drum sample file: {file_name} (ID: {file_id})")
        try:
            ...
            logger.error(f"Storage error creating upload URL for drum sample {file_id}: {str(e)}")
            raise # Re-raise specific storage exceptions
        except Exception as e:
            logger.error(f"Error creating upload URL for drum sample {file_id}: {str(e)}")
            logger.error(traceback.format_exc())
            raise ServiceException(f"Failed to create upload URL for drum sample: {str(e)}")