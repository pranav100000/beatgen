...
async def test_create_melody2():
    ...
    music_tools_service.selected_instruments = {
        "Piano": {"storage_key": "test_piano_key", "id": "test_piano_id"}
    }
    ...
    for i, test_case in enumerate(test_cases):
        ...
        try:
            result = await music_tools_service._handle_create_melody2(
                test_case, tempo, key
            )
            ...
            print(
                f"Created melody with {len(result['instrument']['patterns'][0]['notes'])} notes"
            )
            ...
            for j, note in enumerate(notes[:5]):
                print(
                    f"Note {j + 1}: Pitch {note['pitch']} (MIDI), Start: {note['start']}, Duration: {note['duration']}"
                )
