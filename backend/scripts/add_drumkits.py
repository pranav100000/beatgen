...
if not loaded:
    ...
    logger.warning(
        f".env file not found or failed to load from calculated path: {dotenv_path}. Trying default load_dotenv()."
    )
    loaded = dotenv.load_dotenv()
    if not loaded:
        logger.error(
            "Failed to load .env file from calculated path and default location. Supabase client might fail."
        )
    else:
...
async def create_drum_sample_record(
    ...
    storage_key = (
        f"{category.value}/{sample_id}/{file_name}"  # Use category.value for path
    )
    ...
    logger.info(
        f"Attempting to create drum sample record with ID: {sample_id}, storage_key: {storage_key}, category: {category.name}"
    )
    try:
        result = supabase.execute_query(
            TABLE_NAME, lambda table: table.insert(sample_data)
        )
...
async def upload_and_record_sample(
    ...
    file_format = file_format.lstrip(
        "."
    ).lower()  # Remove leading dot, ensure lowercase
    ...
    logger.info(
        f"Processing sample file: {file_name} for kit: {kit_name} (Category: {category.name})"
    )
    ...
    try:
        ...
        waveform_preview = [
            float(val) for val in rms_normalized
        ]  # Convert to list of standard floats

        logger.debug(
            f"Extracted duration: {duration:.2f}s and waveform preview ({len(waveform_preview)} points) for {file_name}"
        )
        ...
        logger.warning(
            f"Proceeding without duration and waveform data for {file_name}."
        )  # Option 2: Continue
    ...
    try:
        ...
        content_type = (
            f"audio/{file_format}"
            if file_format in ["wav", "mp3", "ogg", "aac", "aiff", "flac"]
        ...
        if response.status_code >= 400:
            logger.error(
                f"Upload failed for {file_name} with status {response.status_code}: {response.text}"
            )
            # Attempt to delete the orphaned DB record
            try:
                logger.warning(
                    f"Attempting to delete orphaned DB record {db_record['id']} due to upload failure."
                )
                supabase.execute_query(
                    TABLE_NAME,
                    lambda table: table.delete().eq("id", db_record["id"]).execute(),
                )
                logger.warning(
                    f"Successfully deleted orphaned DB record: {db_record['id']}"
                )
            except Exception as delete_e:
                logger.error(
                    f"Failed to delete orphaned DB record {db_record['id']}: {delete_e}"
                )
            raise Exception(
                f"Upload failed for {file_name} with status {response.status_code}: {response.text}"
            )
        ...
        logger.error(
            f"Error processing sample {file_name} for kit {kit_name}: {str(e)}"
        )
        raise  # Re-raise the exception to be handled in process_directory


async def process_directory(
    directory_path: str, category: DrumSampleType
) -> List[Dict[str, Any]]:
    """
    ...
    if not os.path.isdir(directory_path):
        logger.error(
            f"Input directory not found or is not a directory: {directory_path}"
        )
        return results
    ...
    logger.info(
        f"Processing directory '{directory_path}' as drum kit '{kit_name}' (Internal: '{internal_kit_name}') in category '{category.name}'"
    )
    ...
    for filename in all_files:
        ...
        if os.path.isfile(file_path) and filename.lower().endswith(
            ...
            try:
                ...
                success_count += 1
                logger.debug(
                    f"Successfully processed and added sample: {filename}"
                )  # Use debug for per-file success
            except Exception:
                fail_count += 1
                # Error is logged within upload_and_record_sample, just note failure here
                logger.warning(
                    f"Failed to process sample {filename}. See previous error. Continuing..."
                )
    ...
    logger.info(
        f"Directory processing completed for '{kit_name}'. Success: {success_count}, Failed: {fail_count}"
    )
    return results
...
async def main(args: argparse.Namespace):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Using category: {category_enum_member.name} (Value: {category_enum_member.value})"
            )
        except KeyError:
            logger.error(
                f"Invalid category: '{args.category}'. Please choose from: {', '.join([m.name for m in DrumSampleType])}"
            )
            parser.print_help()
            sys.exit(1)

        results = await process_directory(
            directory_path=args.directory, category=category_enum_member
        )

        logger.info(
            f"Script finished. Processed {len(results)} samples successfully from directory: {args.directory}"
        )
        ...
        logger.critical(
            f"An unexpected error occurred in main: {str(e)}"
...
""",
    ...
    parser.add_argument(
        "--directory",
        required=True,
        help="Path to the directory containing the drum kit sample files.",
    )
