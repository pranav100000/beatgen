...
async def list_soundfonts(category: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    if category:
        query = lambda table: table.select("*").eq("category", category).order('display_name')
...
async def main(args: argparse.Namespace):
    ...
    try:
        ...
        print("-" * 90)
        print(f"{'ID':<10} | {'Display Name':<25} | {'Category':<15} | {'Storage Key':<35}")
        print("-" * 90)
        ...
        for sf in soundfonts:
            ...
            print(f"{short_id:<10} | {sf.get('display_name', '')[:25]:<25} | {sf.get('category', '')[:15]:<15} | {storage_key:<35}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="List soundfont instruments in the backend")
