...
import {
  ...
  DeleteTrackAction,
  MoveTrackAction,
} from "../core/state/history/actions/TrackActions";
...
function NewProject() {
  ...
  const handleAddTrack = async (trackTypeOrFile: string | File) => {
    ...
    try {
      if (trackTypeOrFile instanceof File) {
        ...
        const audioTrack = await store
          .getAudioEngine()
          .createTrack(newTrack.id, trackTypeOrFile);
        ...
        if (trackTypeOrFile.type.includes("audio")) {
          ...
          console.log("Adding audio track with duration:", {
            duration,
            bpm,
            expectedWidth: duration
              ? calculateTrackWidth(duration, bpm)
              : undefined,
          });
        ...
        const newTrack = store.createTrack(
          trackTypeOrFile,
          trackTypeOrFile as TrackType["type"]
        );
        const audioTrack = await store
          .getAudioEngine()
        ...
        if (trackTypeOrFile === "midi" || trackTypeOrFile === "drum") {
          ...
          const trackTypeLabel =
            trackTypeOrFile === "midi" ? "MIDI" : "Drum Machine";
          console.log(
            `Creating ${trackTypeLabel} track with default duration:`,
            {
              defaultBars,
              totalBeats,
              bpm,
          ...
          if (trackTypeOrFile === "drum") {
            dbId = await db.addDrumMachineTrack(
              newTrack.id,
              `Drum Machine ${tracks.length + 1}`
            );
          }
        }
        ...
        const trackData: TrackState = {
          ...
          _calculatedWidth: defaultDuration
            ? calculateTrackWidth(defaultDuration, bpm)
            : undefined,
          dbId,
        };
  ...
  const handleTrackPositionChange = async (
    trackId: string,
    ...
    if (isDragEnd && !positionsAreEqual(track.position, newPosition)) {
      ...
      setTracks((prev) =>
        prev.map((t) =>
          t.id === trackId ? { ...t, position: newPosition } : t
        )
      );
    }
  };
      ...
      >
        ...
        <TimeSignatureDisplay
          ...
          onTopNumberChange={(value) =>
            handleTimeSignatureChange(value, undefined)
          }
          onBottomNumberChange={(value) =>
            handleTimeSignatureChange(undefined, value)
          }
        />
      ...
      <Box sx={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          ...
          >
            ...
            >
              {/* Vertical grid lines */}
              {/* {Array.from({ length: GRID_CONSTANTS.measureCount + 1 }).map( */}
              ...
              {Array.from({ length: measureCount }).map((_, i) => (
                ...
                // {Array.from({ length: GRID_CONSTANTS.measureCount }).map(
                //   (_, i) => (
                <Box
                  key={`number-${i}`}
                  sx={{
          ...
          {tracks.length > 0 ? (
            ...
            >
              ...
              >
                ...
                {/* {Array.from({
                  length:
                    GRID_CONSTANTS.measureCount *
                    GRID_CONSTANTS.gridSubdivisions,
                }).map((_, i) => { */}
                {Array.from({
                  length:
                    measureCount *
                  ...
                  if (i % GRID_CONSTANTS.gridSubdivisions !== 0) {
                    // Skip positions where major lines exist
                    return (
                      <Box
                        key={`minor-${i}`}
                        sx={{
              ...
              {tracks.map((track, index) => (
                <Track
                  ...
                  onPositionChange={(newPosition, isDragEnd) =>
                    handleTrackPositionChange(track.id, newPosition, isDragEnd)
                  }
                  bpm={bpm}
