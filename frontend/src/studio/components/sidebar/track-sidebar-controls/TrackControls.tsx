...
// import MidiPlayer from '../../../core/audio-engine/midiPlayer';
...
//   const [cachedSoundfonts, setCachedSoundfonts] = useState<Array<{id: string, displayName: string}>>([]);
...
//   // Function to load cached soundfonts
//   const loadCachedSoundfonts = async () => {
//     try {
//       const soundfonts = await db.soundfonts.toArray();
//       const mappedSoundfonts = soundfonts.map(sf => ({
//         id: sf.id,
//         displayName: sf.displayName
//       }));
//       setCachedSoundfonts(mappedSoundfonts);
//     } catch (error) {
//       console.error('Failed to load cached soundfonts:', error);
//     }
//   };
  
//   const handleInstrumentSelect = async (instrumentId: string, displayName: string) => {
//     setInstrumentModalOpen(false);
    
//     if (!onInstrumentChange || track.instrumentId === instrumentId) {
//       return; // No change or no handler
//     }
    
//     try {
//       // Update track state through parent component
//       onInstrumentChange(track.id, instrumentId, displayName);
      
//       // Update the instrument in MidiPlayer
//       const midiPlayer = MidiPlayer.getInstance();
//       await midiPlayer.changeInstrument(track.id, instrumentId);
      
//       // Update MIDI file in IndexedDB to include new instrument
//       if (await db.hasMidiTrack(track.id)) {
//         // Get the MIDI file record
...
//       // Refresh the cached soundfonts list
//       await loadCachedSoundfonts();
//     } catch (error) {
//       console.error(`Failed to change instrument for track ${track.id}:`, error);
//     }
//   };
  
//   // Load cached soundfonts when component mounts
//   useEffect(() => {
//     loadCachedSoundfonts();
//   }, []);
...
//                 if (value === 'browse') {
//                   handleOpenInstrumentModal();
//                 } else {
//                   // Find the selected instrument
//                   const instrument = cachedSoundfonts.find(sf => sf.id === value);
//                   if (instrument && onInstrumentChange) {
//                     handleInstrumentSelect(instrument.id, instrument.displayName);
//                   }
//                 }
//               }}
//               fullWidth
...
//               {/* Recently used soundfonts */}
//               {cachedSoundfonts.length > 0 && (
//                 cachedSoundfonts.map(sf => (
//                   <MenuItem key={sf.id} value={sf.id}>
//                     <Box sx={{ display: 'flex', alignItems: 'center' }}>
//                       <PianoIcon sx={{ mr: 1, fontSize: 16, color: '#6a3de8' }} />
//                       {sf.displayName}
//                     </Box>
//                   </MenuItem>
//                 ))
...
//               {/* Divider if we have cached soundfonts */}
//               {cachedSoundfonts.length > 0 && <MenuItem divider />}
              
//               {/* Browse more option */}
//               <MenuItem value="browse" sx={{ color: '#6a3de8' }}>
//                 <Box sx={{ display: 'flex', alignItems: 'center' }}>
//                   <MusicNoteIcon sx={{ mr: 1, fontSize: 16 }} />
